...
async function createAdmin(username?: string, password?: string, name?: string, email?: string) {
  ...
  try {
    ...
    const existingResult = await pool.query<any>(
      'SELECT id FROM admin_users WHERE username = $1 OR email = $2 LIMIT 1',
    ...
    const result = await pool.query(
      'INSERT INTO admin_users (username, password_hash, name, email, is_super_admin, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [adminUsername, passwordHash, adminName, adminEmail, true, 'active'],
    );
...
async function createSampleCoordinator() {
  ...
  try {
    ...
    const existingResult = await pool.query<any>(
      'SELECT id FROM coordinator_accounts WHERE email = $1 LIMIT 1',
      [email],
    );
    ...
    const result = await pool.query(
      'INSERT INTO coordinator_accounts (access_code, password_hash, full_name, email, department, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [accessCode, passwordHash, fullName, email, department, 'active'],
    );
...
export { createAdmin, createSampleCoordinator };
